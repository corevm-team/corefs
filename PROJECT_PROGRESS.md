...
- [~] Crate `corefs-core` (no_std + alloc) — `domain/`, `config`, `platform`, `error`, `bincode_compat` (Wire-Compat-Layer), `services/*` (außer `integrity` + `compression`), `storage::{allocator, block_device, block_store, catalog, persisted_state, volume_wal}` und 23 von ~30 `storage::ondisk/*`-Modulen vollständig migriert. **Alle 23 ondisk-Module laufen strikt `no_std + alloc`** (seit der bincode-2-Migration sind auch volume/native/grouped/journaled/fsck_repair/reader/scrub keine std-Feature-Voraussetzung mehr). Verbleibend im main crate: `session`, `concurrency`, `benchmark`, `resilience`, `stress`, `property` (alle nutzen `std::{path, sync, thread, time::Instant}`).
