...
- [~] Crate `corefs-core` (no_std + alloc) — `domain/`, `config`, `platform`, `error`, `bincode_compat` (Wire-Compat-Layer), `services/*` (außer `integrity` + `compression`), `storage::{allocator, block_device, block_store, catalog, persisted_state, volume_wal}` (inkl. neuer `PersistedState::empty_at`/`empty`-Konstruktoren) und 24 von ~30 `storage::ondisk/*`-Modulen vollständig migriert: 23 Treiber-/Hilfsmodule (checksum, layout, bitmap, inode, attr_block, dir_entry, xattr, extent_tree, superblock, block_group, refcount, allocator, multi_group_allocator, journal, fault_injection, tiering, fsck, volume, native, grouped, journaled, fsck_repair, reader, scrub) plus eine **service-freie `OdfDeviceSession`** (`format_new_at`, `open`, `mutate`, `flush` — `Box<dyn BlockDevice>` + `PersistedState` ohne `CoreFsService`-Schicht). Verbleibend im main crate: `concurrency`, `benchmark`, `resilience`, `stress`, `property` (nutzen `std::{sync, thread, time::Instant}`) sowie die service-gebundene `OdfDeviceSession`-Variante mit eingebautem `CoreFsService`.
...
- [x] Mutations-Handler Setattr/Rmdir/Rename/Symlink/Readlink — ENOSYS-Stubs ersetzt durch echte `PersistedState`-Mutationen: `op_setattr` wendet mode/uid/gid/size (extend/shrink der `BlockRecord`-Bytes)/mtime/**atime** per `PartialAttr` an; `op_rmdir` prüft Nicht-Leere über `path.starts_with(child_prefix)` und liefert ENOTEMPTY (39); `op_rename` rewrited Quell-Pfad plus alle `old/`-Deszendenten; `op_symlink` legt einen `InodeKind::Symlink` mit UTF-8-Target im `BlockRecord` an; `op_readlink` dekodiert die Bytes als String. 7 neue Handler-Unit-Tests. Parallel dazu 6 host-runnable E2E-Tests in `tests/fuse_handler_e2e.rs`, die dieselben Mutation-Flows gegen eine frische `OdfDeviceSession` fahren.
- [x] POSIX-Rename-Overwrite-Semantik — `op_rename` (corefsd-Handler) und `rename_entry` (Kernel-`CoreFsDriver`) implementieren atomic overwrite: file-over-file ersetzt das Ziel, dir-over-empty-dir entfernt das leere Ziel vor dem Umbenennen, dir-over-non-empty-dir liefert `ENOTEMPTY`, file-over-dir `EISDIR`, dir-over-file `ENOTDIR`. Neuer `FsError::DirectoryNotEmpty` + errno-Mapping auf `ENOTEMPTY=39`. 5 neue Handler-Unit-Tests + 5 Driver-Compile-Tests.
- [x] `accessed_at`-Persistenz (atime) — Domain-`Inode` erhält ein viertes POSIX-Timestamp-Feld (`accessed_at`) neben `created_at`/`modified_at`/`changed_at`. Wire-Compat via `#[serde(default)]` für bestehende bincode-Volumes. `Inode::touch_accessed_at(now)` isoliert atime vom ctime/mtime-Pfad. `op_setattr` honoriert `PartialAttr::atime_secs` ohne mtime/ctime zu berühren; `op_read` ruft `touch_accessed_at` (strict atime; Relatime/Noatime-Layer später). Neue Tests in `inode_tests.rs` + Handler (`setattr_atime_persists_on_inode`).
